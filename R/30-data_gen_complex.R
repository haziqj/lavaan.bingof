...
make_population_new <- function(model_no = 1, seed = 123, H1 = FALSE,
                            return_all = FALSE) {
  ...
  nschools       <- c(400, 1000, 600)  # types A, B, C
  avg_class_size <- c(15, 25, 20)
  avg_nstudents  <- c(200, 500, 700)  # so pop total is nschools x avg_nstudent
  sd_nstudents   <- 25
  ...
  pop <-
    tibble(type = LETTERS[1:3],
           nschools = nschools,
           avg_class_size = avg_class_size,
           avg_nstudent = avg_nstudents) %>%
    rowwise() %>%
    # mutate(school = list(sprintf("%04d", seq_len(nschools)))) %>%
    mutate(school = list(seq_len(nschools))) %>%
    unnest_longer(school) %>%
    mutate(nstudents = round(rnorm(dplyr::n(), mean = avg_nstudent,
                                   sd = sd_nstudents))) %>%
    rowwise() %>%
    mutate(class = list(sample(letters[seq_len(nstudents / avg_class_size)],
                               size = nstudents, replace = TRUE))) %>%
...
gen_data_bin_complex3 <- function(population = make_population(1, seed = NULL),
  ...
  set.seed(seed)
  if (!missing(n)) npsu <- round(n / (5 * 3), 0)
  ...
  school_info <-
    ...
    summarise(
      pr_student_selected = 5 / dplyr::n(),
      .groups = "drop"
    ) %>%
    group_by(school) %>%  # clusters
    mutate(pr_class_selected = 1 / length(unique(class))) %>%
    group_by(type) %>%  # stratum
    mutate(pr_school_selected = npsu / dplyr::n(),
           prob = pr_school_selected * pr_class_selected * pr_student_selected,
           wt = 1 / prob)
  ...
  sampled <-
    ...
    ungroup() %>%
    left_join(school_info, by = c("type", "school", "class")) %>%
    select(type, school, class, wt, starts_with("y")) %>%
    mutate(wt = wt / sum(wt) * dplyr::n(),
           across(starts_with("y"), ordered)) %>%
    arrange(type, school, class)
