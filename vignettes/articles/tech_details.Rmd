...
$$
X^2 = n \hat\be_2^\top \hat\bW \be_2,
$$
where $\hat\bW \xrightarrow{\text P} \bW$ is some $S\times S$ weight matrix.
We summarise the various weight matrices that we use in this R package:

|   | Name        | Weight ($\bW$)     | D.f.  | R function          | Remarks                           |
|---|-------------|---------------------|-------|---------------------|-----------------------------------|
...
| 3 | Wald V3     | $\bW\bOmega_2\bW$ | $S-m$ | `Wald_vcovf_test()`    | $\bOmega_2$ need not be estimated |
