...
Let $\cS{ab}$ be the set of sample units contained within PSU $b$ within stratum $a$.
Define
\begin{equation}
\bu_{ab} = \sum_{h \in \cS{ab}} w_h (\by^{(h)} - \hat\bpi).
\end{equation}
...
1. (Stratified sampling) There are three strata $a=1,2,3$ corresponding to the school types, and the PSUs are the individual students within schools in each stratum. An equal size is taken per stratum, so $n_a=1000$, and thus the sample units $\cS{ab}$ lists out the students from various schools and classes within each stratum $a$ (there is no sum over $b$).
...
3. (Two-stage stratified cluster sampling) Same as 1, but the sample units $\cS{ab}$ lists out the students from school $b$ (from the selected class)  within each stratum $a$.
