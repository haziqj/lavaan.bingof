...
$$
The total sample size will vary from sample to sample, but on average will be $n = 140 \times 21.5 = 3010$, where $\frac{15 \times 400 + 25 \times 1000 + 20 \times 600} = 21.5$ is the average class size per school.
...
Note: The $\bSigma$ matrix appearing in the plots below have been standardised so the plots show correlation matrices instead. These are not the tetrachoric correlations, instead they are correlations assuming that the responses are numeric 0s and 1s.
...
ggcorrplot(Sigma, type = "lower", outline.col = "white",
           ggtheme = ggplot2::theme_bw, lab = TRUE,
           colors = c("#455BCDFF", "white", "#C42503FF")) +
  labs(title = expression("Population"~Sigma))
```
...
rownames(Sigma3) <- colnames(Sigmay) <- paste0("y", 1:5)
Sigma3 <- diag(1 / sqrt(diag(Sigma3))) %*% Sigma3 %*% diag(1 / sqrt(diag(Sigma3)))
ggcorrplot(Sigma3, type = "lower", outline.col = "white",
           ggtheme = ggplot2::theme_bw, lab = TRUE,
           colors = c("#455BCDFF", "white", "#C42503FF")) +
  labs(title = expression("Stratified cluster sampling"~Sigma~"estimate"))
```
